// BDI University — daily subcontractor-insurance reminder (Vercel Cron).
//
...
//
// Sends from the admin's connected Outlook (ADMIN_EMAIL).
// Env: MS_CLIENT_ID, MS_CLIENT_SECRET, MS_TENANT_ID, ADMIN_EMAIL, Firebase SA vars, CRON_SECRET.
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!subs.length || !recipients.length) { log.note = "nothing to do (no subcontractors or no recipients)"; await save(notified); res.status(200).json(log); return; }
    ...
    log.due = due.length;
    if (!due.length) { await save(notified); res.status(200).json(log); return; }
    ...
    if (!adminTok) { log.errors.push("admin Outlook not connected (set ADMIN_EMAIL and connect Outlook for that account)"); await save(notified); res.status(200).json(log); return; }
    ...
    due.sort((a, b) => a.daysLeft - b.daysLeft).forEach(function (d) {
      ...
      lines.push("  - " + (s.name || "(unnamed)") + (s.coverage ? " [" + s.coverage + "]" : "") + " — " + status + " (expires " + s.expires + ")" + (s.carrier ? " — " + s.carrier : ""));
    });
    lines.push("");
